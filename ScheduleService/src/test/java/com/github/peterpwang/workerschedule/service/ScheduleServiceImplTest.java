...
public class ScheduleServiceImplTest {
	...
	public void setUp() {
		...
		Mockito.when(scheduleRepository.save(any(Schedule.class))).thenAnswer(
			invocation -> invocation.getArgument(0));
		Mockito.doNothing().when(scheduleRepository).deleteById(SCHEDULE_ID_1);
	...
	@Test
	public void whenValidIdThenScheduleShouldBeFound() {
	...
	@Test
	public void whenInValidIdThenScheduleShouldNotBeFound() {
	...
	@Test
	public void given3SchedulesWhenGetAllThenReturn3Records() {
		Schedule schedule1 = Util.newSchedule(SCHEDULE_ID_1, SCHEDULE_NAME_1);
		Schedule schedule2 = Util.newSchedule(SCHEDULE_ID_2, SCHEDULE_NAME_2);
		Schedule schedule3 = Util.newSchedule(SCHEDULE_ID_3, SCHEDULE_NAME_3);
		...
		assertThat(list).hasSize(3).extracting(Schedule::getName).contains(schedule1.getName(), schedule2.getName(),
				schedule3.getName());
	}

	@Test
	public void createThenSuccess() {
		Schedule schedule1 = Util.newSchedule(SCHEDULE_ID_1, SCHEDULE_NAME_1);
		Schedule savedSchedule = scheduleService.save(schedule1);
		assertThat(savedSchedule.getName()).isEqualTo(SCHEDULE_NAME_1);
	}

	@Test
	public void whenValidIdThenScheduleUpdate() {
		Optional<Schedule> fromDb = scheduleService.findById(SCHEDULE_ID_1);
		assertThat(fromDb.isPresent()).isEqualTo(true);
		
		Schedule schedule1 = fromDb.get();
		schedule1.setName(SCHEDULE_NAME_2);
		Schedule savedSchedule = scheduleService.save(schedule1);
		assertThat(savedSchedule.getName()).isEqualTo(SCHEDULE_NAME_2);
	}

	@Test
	public void whenValidIdThenScheduleDelete() {
		Optional<Schedule> fromDb = scheduleService.findById(SCHEDULE_ID_1);
		assertThat(fromDb.isPresent()).isEqualTo(true);
